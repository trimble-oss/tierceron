...
func SeedVaultById(goMod *helperkv.Modifier, service string, address string, token string, baseTemplate *extract.TemplateResultData, tableData map[string]interface{}, tableId string, logger *log.Logger, project string) error {
	...
	il.SeedVaultFromData(true, []byte(seedData), address, token, goMod.Env, log.Default(), service, false, goMod.Env+"/Index/"+project+"/"+tableId)
	return nil
}
