...
config:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go install -gcflags=-G=0  tierceron/trcconfig
...
configwin:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) GOOS=windows GOARCH=amd64 go build -gcflags=-G=0  -o $(GOBIN)/trcconfig.exe trcconfig/trcconfig.go
configmac:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) GOOS=darwin GOARCH=amd64 go build -gcflags=-G=0  -o $(GOBIN)/trcconfig.mac tierceron/trcconfig
seed:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go install -gcflags=-G=0  tierceron/trcinit
seedmac:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) GOOS=darwin GOARCH=amd64 go build -gcflags=-G=0  -o $(GOBIN)/trcinit.mac tierceron/trcinit 
seedp:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go install -gcflags=-G=0  tierceron/trcinitp
x:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go install -gcflags=-G=0  tierceron/trcx
xmac:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) GOOS=darwin GOARCH=amd64 go build -gcflags=-G=0  -o $(GOBIN)/trcx.mac tierceron/trcx
xlib:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) GOOS=linux GOARCH=amd64 go build -gcflags=-G=0  -buildmode=c-shared -a -ldflags '-w' -o $(GOBIN)/nc.so tierceron/configlib
maclib:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 go build -gcflags=-G=0  -buildmode=c-shared -o $(GOBIN)/nc.dylib tierceron/configlib
winlib:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) CGO_ENABLED=1 GOOS=windows GOARCH=amd64 CC=x86_64-w64-mingw32-gcc go build -gcflags=-G=0  -buildmode=c-shared -o $(GOBIN)/nc.dll tierceron/configlib
xp:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go install -gcflags=-G=0  tierceron/trcxp
pub:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go install -gcflags=-G=0  tierceron/trcpub
sub:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go install -gcflags=-G=0 tierceron/trcsub
certify:
	@GOPATH=$(GOPATH) GOGC=off GOBIN=$(GOBIN) go build -gcflags=-G=0 -o $(GOBIN)/trcplgtool -tags "awsecr" tierceron/trcvault/trcplgtoolbase
gen:
