...
if [ "$SECRETS_ONLY" = "Y" ] || [ "$SECRETS_ONLY" = "yes" ] || [ "$SECRETS_ONLY" = "y" ]; then
    ...
    SECRET_ENV_TOKEN=$VAULT_ENV_TOKEN
fi

vault write $TRC_PLUGIN_NAME/$VAULT_ENV token=$VAULT_ENV_TOKEN vaddress=$VAULT_API_ADDR caddress=$SECRET_VAULT_ADDR ctoken=$SECRET_ENV_TOKEN plugin=$TRC_PLUGIN_NAME
