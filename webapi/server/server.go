...
func (s *Server) GetServicePaths(ctx context.Context, req *pb.ServicePathReq) (*pb.ServicePathResp, error) {
	// Connect to the vault
	mod, err := kv.NewModifier(s.VaultToken, s.VaultAddr, s.CertPath)
	mod.Env = req.Env
	...
	//data, err := getPathsFromService(mod, req.Service)
	paths := []string{}
	secret, err := mod.List(req.Service + "/")
	if err != nil {
		//fmt.Println("error")
		return nil, err
	}
	if secret != nil {
		//fmt.Println("yes secrets")
		pathsI := secret.Data["keys"].([]interface{})
		for _, pathI := range pathsI {
			p := pathI.(string)
			path := req.Service + "/" + p
			//fmt.Println(path)
			paths = getPaths(mod, path)
			//data = append(data, d)
		}
	} else {
		fmt.Println("no paths")
	}
	return &pb.ServicePathResp{
		Paths: paths,
	}, nil
}
func (s *Server) GetPathData(ctx context.Context, req *pb.PathDataReq) (*pb.PathDataResp, error) {
	// Connect to the vault
	mod, err := kv.NewModifier(s.VaultToken, s.VaultAddr, s.CertPath)
	mod.Env = req.Env
	//fmt.Println("mod: " + mod.Env)
	...
	if len(req.Paths) > 0 {
		for _, path := range req.Paths {
			...
			//fmt.Println("no data at path " + path)
			//}
		}
	} else {
		fmt.Println("no paths")
	}
	return &pb.PathDataResp{
		Data: data,
	}, nil
}
