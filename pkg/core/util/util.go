...
type (
	ProcessFlowConfig     func(pluginEnvConfig map[string]any) map[string]any
	ProcessFlowInitConfig func(flowMachineInitContext *flowcore.FlowMachineInitContext, pluginConfig map[string]any, logger *log.Logger) error
	BootFlowMachineFunc   func(flowMachineInitContext *flowcore.FlowMachineInitContext, driverConfig *config.DriverConfig, pluginConfig map[string]any, logger *log.Logger) (any, error)
)
...
func UncompressZipFile(filePath string) (bool, []error) {
	...
	if readErr != nil {
		fmt.Fprintln(os.Stderr, "Could not open zip file -  "+readErr.Error())
	...
	for _, f := range r.File {
		...
		if strings.Contains(f.Name, "..") {
			fmt.Fprintln(os.Stderr, "Path must be absolute in archive - "+f.Name+".")
			errorList = append(errorList, fmt.Errorf("path must be absolute in archive - %s", f.Name))
			return false, errorList
		}
		rc, openErr := f.Open()
		if openErr != nil {
			fmt.Fprintln(os.Stderr, "Could not open file inside archive - "+f.Name+" - "+openErr.Error())
		...
		if f.FileInfo().IsDir() {
			dirErr := os.MkdirAll(newFilePath, 0o700)
			if dirErr != nil {
				fmt.Fprintln(os.Stderr, "Could not create directory  - "+dirErr.Error())
		...
		if createErr != nil {
			fmt.Fprintln(os.Stderr, "Could not open create uncompressed file - "+createErr.Error())
			errorList = append(errorList, createErr)
		}
		_, uncompressErr := io.Copy(uncompressedFile, rc)
		if uncompressErr != nil {
			fmt.Fprintln(os.Stderr, "Could not copy uncompressed file into directory - "+uncompressErr.Error())
