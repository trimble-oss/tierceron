...
func getCertPathOverride(certOverrides map[string]string, filename string) string {
	if len(certOverrides) == 0 {
		return ""
	}

	if override := certOverrides[filename]; override != "" {
		return override
	}
	...
	if slashIndex := strings.LastIndex(filename, "/"); slashIndex >= 0 {
		...
		if legacyBaseName := getLegacyTemplateKey(baseName); legacyBaseName != baseName {
			legacyFilename = filename[:slashIndex+1] + legacyBaseName
		}
	} else if legacyKey := getLegacyTemplateKey(filename); legacyKey != filename {
		legacyFilename = legacyKey
	}

	if legacyFilename != filename {
		return certOverrides[legacyFilename]
	}

	return ""
}

func getLegacyTemplateKey(baseName string) string {
...
func populateCertFileData(driverConfig *config.DriverConfig, certData map[int]string, certSourcePath string, certBundleJks string, hasCertBundleJks bool, certPasswordVaultPath any, hasCertPasswordVaultPath bool, decoded []byte) (map[int]string, error) {
	certData[1] = string(decoded)
	certData[2] = certSourcePath
	...
	if hasCertBundleJks && driverConfig.WantKeystore != "" {
		...
		if ksErr := validator.AddToKeystore(driverConfig, certSourcePath, []byte(certPassword), certBundleJks, decoded); ksErr != nil {
			return nil, ksErr
		}
	}
	...
	if ok {
		...
		if cert {
			if serviceValues, ok := values[serviceLookup]; ok {
				...
				if certPathOverride := getCertPathOverride(driverConfig.CertPathOverrides, filename); certPathOverride != "" {
					certDestPath = certPathOverride
				}
				...
				if hasCertDefinition && hasCertSourcePath {
					...
					if certData, err = populateCertFileData(driverConfig, certData, certSourcePath.(string), func() string {
						if hasCertBundleJks {
							return certBundleJks.(string)
						}
						return ""
					}(), hasCertBundleJks, certPasswordVaultPath, hasCertPasswordVaultPath, decoded); err != nil {
						eUtils.LogErrorObject(driverConfig.CoreConfig, err, false)
						return "", nil, err
					}
					return "", certData, nil
