...
From command line, navigate to the directory containing terraform files (vaultinit/vaultfiles) and run `terraform init`
...
Exit/kill apiRouter 
Start vaultapi service
Log into UI using webapi token generated from seed process

##Starting the server
Navigate to /etc/opt/vaultAPI and run `sudo ./apiRouter`
...
Seperate values for different environments in different files. [Example](Vault.Whoville/vault_seeds/seed.yml)
...
#Run Vault Init 
Token is the only flag that needs to be specified to run. Also, when running vaultinit in the local environment, user will be prompted for environment, username, and password.
...
vaultinit
	...
	-log string
		Output path for log files (default "/var/log/vaultinit.log")
	-new
	...
	-seeds string
		Directory that contains vault seeds (default "vault_seeds")
	-shard string
...
```
#Run Vault Config
Run in the ServiceTechAPI folder and check the hibernate.properties file generated by vaultconfig to verify that the vault has been reseeded.
Will not run if you don't have appRoleID and secretID tokens. 

**CLI Syntax:**

*	`vaultconfig` 
    Executes vaultconfig. If the cert file exists vaultconfig will use appRoleID and secretID tokens from the file. If no cert file exists vaultconfig will prompt for appRoleID and secretID then create a cert file and save those tokens to it for future use.

*	`vaultconfig -appRoleID=foo -secretID=bar`
...
vaultconfig
	...
	-addr string
		...
		Specified in vault's config file
	-startDir string
		Template directory (default "vault_templates")
	-endDir string
	...
	-tokenfiles string
		Directory with yaml files used to generate tokens (default "token_files")
	-tokenName string
		Token name used by this vaultconfig to access the vault
```

# Bamboo
vaultconfig and vaultpub are built and shared artifacts from the 'Vault Tools' build along with the vault's public certificate. 
...
The repository was created using the MEP Service template and it adopts the default microservice architecture.
For any additions, the same structure should be followed.
