...
type MemBuilder struct {
	...
	restMapperFn resource.RESTMapperFunc
	...
	selectAll         bool
	limitChunks       int64
	requestTransforms []resource.RequestTransform
...
func (b *MemBuilder) Path(recursive bool, paths ...string) *MemBuilder {
	for _, p := range paths {
		...
		visitors, err := b.ExpandPathsToFileVisitors(p, recursive, resource.FileExtensions)
		if err != nil {
...
func (b *MemBuilder) visitBySelector() *Result {
	...
	for _, mapping := range mappings {
		...
		visitors = append(visitors, resource.NewSelector(client, mapping, selectorNamespace, labelSelector, fieldSelector, b.limitChunks))
	}
	if b.continueOnError {
		result.visitor = resource.EagerVisitorList(visitors)
	} else {
		result.visitor = resource.VisitorList(visitors)
	}
...
func (b *MemBuilder) visitByResource() *Result {
	...
	clients := make(map[string]resource.RESTClient)
	for _, mapping := range mappings {
	...
	if b.continueOnError {
		visitors = resource.EagerVisitorList(items)
	} else {
		visitors = resource.VisitorList(items)
	}
...
func (b *MemBuilder) visitByPaths() *Result {
	...
	var visitors resource.Visitor
	if b.continueOnError {
		visitors = resource.EagerVisitorList(b.paths)
	} else {
		visitors = resource.VisitorList(b.paths)
	}
	...
	if b.latest {
		...
		if b.defaultNamespace {
			visitors = resource.NewDecoratedVisitor(visitors, resource.SetNamespace(b.namespace))
		}
		visitors = resource.NewDecoratedVisitor(visitors, resource.RetrieveLatest)
	}
	if b.labelSelector != nil {
		...
		visitors = resource.NewFilteredVisitor(visitors, resource.FilterByLabelSelector(selector))
	}
	result.visitor = visitors
...
func (b *MemBuilder) Do() *Result {
	...
	helpers := []resource.VisitorFunc{}
	if b.defaultNamespace {
		helpers = append(helpers, resource.SetNamespace(b.namespace))
	}
	if b.requireNamespace {
		helpers = append(helpers, resource.RequireNamespace(b.namespace))
	}
	helpers = append(helpers, resource.FilterNamespace)
	if b.requireObject {
		helpers = append(helpers, resource.RetrieveLazy)
	}
	if b.continueOnError {
		r.visitor = resource.NewDecoratedVisitor(resource.ContinueOnErrorVisitor{r.visitor}, helpers...)
	} else {
		r.visitor = resource.NewDecoratedVisitor(r.visitor, helpers...)
	}
	return r
}
...
type cachingRESTMapperFunc struct {
	delegate resource.RESTMapperFunc
...
type cachingCategoryExpanderFunc struct {
	delegate resource.CategoryExpanderFunc
