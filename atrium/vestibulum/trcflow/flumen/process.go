...
func ProcessFlows(pluginConfig map[string]interface{}, logger *log.Logger) error {
	...
	for _, table := range GetTierceronTableNames() {
		tfContext := trcflowcore.TrcFlowContext{RemoteDataSource: make(map[string]interface{}), ReadOnly: false, Init: true, Log: tfmContext.DriverConfig.CoreConfig.Log, ContextNotifyChan: make(chan bool, 1)}
	...
	for _, table := range driverConfigBasis.VersionFilter {
		...
		go func(tableFlow flowcore.FlowNameType, dc *config.DriverConfig) {
			...
			defer flowWG.Done()
			tfContext := trcflowcore.TrcFlowContext{RemoteDataSource: map[string]interface{}{}, FlowLock: &sync.Mutex{}, ReadOnly: false, Init: true, Log: tfmContext.DriverConfig.CoreConfig.Log, ContextNotifyChan: make(chan bool, 1)}
