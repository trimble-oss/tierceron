...
func receiver(receive_chan chan tccore.KernelCmd) {
	for {
		...
		case event.Command == tccore.PLUGIN_EVENT_STOP:
			...
			sender <- errors.New("viride shutting down")
			return
...
func init() {
	...
	fmt.Fprintf(os.Stderr, "viride Version: %s\n", sha)
}

func send_dfstat() {
	if configContext == nil || configContext.DfsChan == nil || dfstat == nil {
		fmt.Fprintln(os.Stderr, "Dataflow Statistic channel not initialized properly for viride.")
		return
	}
...
func send_err(err error) {
	if configContext == nil || configContext.ErrorChan == nil || err == nil {
		fmt.Fprintln(os.Stderr, "Failure to send error message, error channel not initialized properly for viride.")
		return
	}
	if dfstat != nil {
		dfsctx, _, err := dfstat.GetDeliverStatCtx()
...
func start(pluginName string) {
	if configContext == nil {
		fmt.Fprintln(os.Stderr, "no config context initialized for viride")
		return
	}
...
func stop(pluginName string) {
	if configContext != nil {
		configContext.Log.Println("viride received shutdown message from kernel.")
	}
	if configContext != nil {
		configContext.Log.Println("Stopped server for viride.")
		dfstat.UpdateDataFlowStatistic("System",
			pluginName,
			"Shutdown",
			"0",
