...
import (
	...
	"github.com/trimble-oss/tierceron/atrium/vestibulum/hive/plugins/trcspiralis/hcore"
	"gopkg.in/yaml.v2"
)
...
func main() {
	logFilePtr := flag.String("log", "./trcspiralis.log", "Output path for log file")
	flag.Parse()
	...
	logger := log.New(f, "[trcspiralis]", log.LstdFlags)
	config["log"] = logger
	...
	Init("spiralis", &config)
	hcore.GetConfigContext("spiralis").Start("spiralis")
}
