...
#QA
trcplgtool -addr=$VAULT_ADDR -token=$VAULT_TOKEN -env=QA -defineService -pluginName=trcdb -trcbootstrap=/deploy/deploy.trc -projectservice="Hive/PluginTrcdb" -pluginType=trcflowpluginservice -codeBundle=trcdb.so -deployroot=/usr/local/trcshk -deploysubpath=plugins
