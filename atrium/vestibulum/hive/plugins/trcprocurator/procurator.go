...
import (
	...
	tcore "github.com/trimble-oss/tierceron/atrium/vestibulum/hive/plugins/trcprocurator/tcore"

	"gopkg.in/yaml.v2"
)

func GetConfigPaths(pluginName string) []string {
	return tcore.GetConfigPaths(pluginName)
}

func Init(pluginName string, properties *map[string]interface{}) {
	tcore.Init(pluginName, properties)
}

func main() {
	logFilePtr := flag.String("log", "./trcprocurator.log", "Output path for log file")
	flag.Parse()
	...
	logger := log.New(f, "[trcprocurator]", log.LstdFlags)
	config["log"] = logger
	...
	config[tcore.COMMON_PATH] = &configCommon
	...
	Init("procurator", &config)

	tcore.GetConfigContext("procurator").Start("procurator")
	time.Sleep(5 * time.Second)
	msg := tcore.ProcuratorDiagnostic()
